...
const ViewDeckContentWithIdentifier = () => {
  ...
  const loadDeckData = useCallback(
    async (token: string, accessToken: string | null) => {
      setLoading(true);
      try {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      try {
        ...
        const reqResponse = await fetch(
          `/api/access/requirements?token=${token}`,
        );
        ...
        if (storedAccessToken) {
          ...
          const verifyUrl =
            linkIdentifier || requirements.link_identifier
    ...
    checkAccess();
  }, [linkIdentifier, loadDeckData, token]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <p>
              This link is invalid or has expired. Please check the URL and try
              again.
            </p>
          </CardContent>
        </Card>
...
export default function ViewDeckPageWithIdentifierClient() {
  return (
    <Suspense
      fallback={
        <div className="fixed inset-0 bg-black flex items-center justify-center">
