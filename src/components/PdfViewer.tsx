...
const PDFViewer = React.memo<PDFViewerProps>(
  ({ pdfLink, isDownloadable, token, accessToken }) => {
    ...
    useEffect(() => {
      ...
      const trackPageView = async () => {
        ...
        const durationSeconds = Math.max(
          0,
          Math.round((now - analyticsSession.pageStartedAt) / 1000),
        );
    ...
    useEffect(() => {
      return () => {
        ...
        const now = Date.now();
        const currentPageDuration = Math.max(
          0,
          Math.round((now - analyticsSession.pageStartedAt) / 1000),
        );
        ...
        const totalDuration = Math.max(
          0,
          Math.round((now - analyticsSession.startTime) / 1000),
        );
        ...
        if (navigator.sendBeacon) {
          const pageBlob = new Blob([currentPagePayload], {
            type: "application/json",
          });
          navigator.sendBeacon("/api/analytics/track", pageBlob);

          const viewBlob = new Blob([viewEndPayload], {
            type: "application/json",
          });
          ...
          fetch("/api/analytics/track", {
            ...
            console.error("Failed to track final page view:", err),
          );
    ...
    return (
      <div className="fixed inset-0 bg-black text-white select-none overflow-hidden">
        ...
        <div
          className={`absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/80 to-transparent p-4 transition-opacity duration-300 ${isFullscreen ? "opacity-0 hover:opacity-100" : ""}`}
        >
        ...
        {/* Navigation Buttons */}
        <div
          className={`absolute ${isMobile ? "left-4 right-4 bottom-24" : "left-4 right-4 top-1/2 -translate-y-1/2"} flex items-center justify-between z-20 transition-opacity duration-300 ${isFullscreen ? "opacity-0 hover:opacity-100" : ""}`}
        >
          <Button
            variant="ghost"
          ...
          <Button
            ...
            onClick={nextPage}
            disabled={
              viewerState.pageNumber >= viewerState.numPages || isLoading
            }
            className="text-white hover:bg-white/20 disabled:opacity-30 bg-black/50 backdrop-blur-sm rounded-full shadow-lg transition-all duration-200 hover:scale-105"
            style={
              isMobile
        ...
        <div
          className={`absolute ${isMobile ? "bottom-4" : "bottom-0"} left-0 right-0 z-10 bg-gradient-to-t from-black/80 to-transparent p-4 transition-opacity duration-300 ${isFullscreen ? "opacity-0 hover:opacity-100" : ""}`}
        >
        ...
        {!isDownloadable && (
          <div
            className={`absolute ${isMobile ? "bottom-24" : "bottom-4"} left-4 text-xs text-gray-500 opacity-50 pointer-events-none select-none`}
          >
